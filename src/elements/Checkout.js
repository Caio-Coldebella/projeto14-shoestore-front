...
export default function Checkout(){
    ...
    useEffect(()=>{
        axios.get(`${process.env.REACT_APP_API_BASE_URL}/checkout`,{headers:{Authentication:token}})
        .then((res)=>{setData(res.data); sumitems(res.data,setTotal)}).catch((err)=>{
    ...
    return(
        ...
        <style.CONTENT>
            ...
            {data.length>0?
            data.map((item,index)=>{return <style.ITEM key={index}>{
                <>
            ...
            <style.BOTTOMBAR>
                ...
                <style.CONFIRM onClick={()=>{navigate("/completed")}}>
                    <Cart4 size={40}/>
                </style.CONFIRM>
            </style.BOTTOMBAR>
        </style.CONTENT>
